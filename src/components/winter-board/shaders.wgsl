...
// Reason: Snowflake shape using 6-fold symmetry
fn sdSnowflake(p : vec2f) -> f32 {
  // Convert to polar coordinates
  let angle = atan2(p.y, p.x);
  let radius = length(p);

  // 6-fold symmetry: map angle to [0, PI/3]
  let sector = abs(((angle + 3.14159265) % 1.0471975) - 0.5235988);

  // Create the main arm shape
  let armWidth = 0.08;
  let armLength = 0.45;

  // Convert back to cartesian for this sector
  let localX = radius * cos(sector);
  let localY = radius * sin(sector);

  // Main arm (line along x-axis in local coords)
  let mainArm = abs(localY) - armWidth * (1.0 - localX * 0.5);

  // Branch patterns on the arm
  let branchPos1 = 0.25;
  let branchPos2 = 0.38;
  let branchAngle = 0.6;
  ...
  let b1Arm = abs(localY - b1x * branchAngle) - armWidth * 0.6;
  let b1ArmNeg = abs(localY + b1x * branchAngle) - armWidth * 0.6;

  // Second branch
  let b2x = localX - branchPos2;
  let b2Arm = abs(localY - b2x * branchAngle * 0.8) - armWidth * 0.4;
  let b2ArmNeg = abs(localY + b2x * branchAngle * 0.8) - armWidth * 0.4;

  // Combine: inside arm length and (main arm OR branches)
  let inArm = select(1.0, mainArm, localX > 0.0 && localX < armLength);
  let inBranch1 = select(1.0, min(b1Arm, b1ArmNeg), b1x > 0.0 && b1x < 0.15);
  let inBranch2 = select(1.0, min(b2Arm, b2ArmNeg), b2x > 0.0 && b2x < 0.1);

  // Center hexagon
  let centerHex = radius - 0.12;

  return min(min(min(inArm, inBranch1), inBranch2), centerHex);
}
...
fn updateParticles(@builtin(global_invocation_id) globalInvocationId : vec3u) {
  ...
  if (particle.spawned == 0 || particle.position.y > uniforms.viewportSize.y) {
    if (atomicSub(&simulationCtx.particlesToSpawn, 1) > 0) {
      ...
      let baseOpacity = select(0.8, 1.0, nearCamera);
